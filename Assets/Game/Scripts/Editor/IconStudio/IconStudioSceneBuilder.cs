...
public static class IconStudioSceneBuilder
{
    const string ScenePath = "Assets/Game/Scenes/IconStudio.unity";

    [MenuItem("Game/Icon Studio/Create Or Update Icon Studio Scene")]
    public static void CreateOrUpdateScene()
    {
        ...
        var controller = new GameObject("IconStudio").AddComponent<IconStudioController>();
        ...
        EditorUtility.DisplayDialog(
            "Icon Studio",
            $"场景已保存到:\n{ScenePath}\n\n打开后 Play，使用下拉框/AD键切换装备，左键拖拽旋转，F5 单张渲染，F6 批量渲染。\n\n装备会先穿在骨骼模特上，再自动隐藏模特，只显示当前部位用于图标渲染。",
            "OK");
    }
    ...
    static void ConfigureCamera()
    {
        ...
        camera.gameObject.name = "IconPreviewCamera";
    ...
    static void ConfigureLighting()
    {
        ...
        keyLightGo.transform.rotation = Quaternion.Euler(38f, -35f, 0f);

        var fillLightGo = new GameObject("Fill Light");
        var fillLight = fillLightGo.AddComponent<Light>();
        fillLight.type = LightType.Directional;
        fillLight.intensity = 0.45f;
        fillLight.color = new Color(0.82f, 0.9f, 1f);
        fillLightGo.transform.rotation = Quaternion.Euler(10f, 140f, 0f);
    ...
    {
        var stage = new GameObject("IconPreviewStage");
        stage.transform.position = Vector3.zero;
        stage.transform.rotation = Quaternion.Euler(IconStudioSettings.DefaultFrontEuler);
        return stage.transform;
    }

    static void CreateUi()
    {
        var canvasGo = new GameObject("IconStudioUI");
        ...
        var title = CreateLabel(panel.transform, "TitleLabel", new Vector2(0f, -18f), new Vector2(760f, 36f), 24);
        title.text = "Icon Studio — 装备 UI 渲染";

        var dropdown = CreateDropdown(panel.transform, "EntryDropdown", new Vector2(0f, -72f), new Vector2(760f, 42f));
    ...
    static void WireControllerReferences(IconStudioController controller)
    {
        var so = new SerializedObject(controller);
        so.FindProperty("previewCamera").objectReferenceValue = Camera.main;
        so.FindProperty("previewStage").objectReferenceValue = GameObject.Find("IconPreviewStage")?.transform;
        so.FindProperty("keyLight").objectReferenceValue = GameObject.Find("Key Light")?.GetComponent<Light>();
    ...
    static TMP_Dropdown CreateDropdown(Transform parent, string name, Vector2 anchoredPosition, Vector2 size)
    {
        ...
        var arrowLabel = arrowGo.AddComponent<TextMeshProUGUI>();
        arrowLabel.text = "▼";
