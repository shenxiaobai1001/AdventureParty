...
public static class EquipmentSlotUtility
{
    ...
    public static bool TryParseItemId(int itemId, out int setId, out SyntyEquipmentSlot slot)
    {
        ...
        slot = NormalizeSlot((SyntyEquipmentSlot)slotValue);
        return setId > 0;
    }

    public static SyntyEquipmentSlot NormalizeSlot(SyntyEquipmentSlot slot)
    {
        return SyntyEquipmentPartClassifier.NormalizeSlot(slot);
    }

    public static string GetSlotLabel(SyntyEquipmentSlot slot)
    {
        return IconRenderEntry.GetSlotLabel(NormalizeSlot(slot));
    }
    ...
    public static bool TryParseSlot(string slotName, out SyntyEquipmentSlot slot)
    {
        ...
        if (!System.Enum.TryParse(slotName.Trim(), true, out slot))
            return false;
