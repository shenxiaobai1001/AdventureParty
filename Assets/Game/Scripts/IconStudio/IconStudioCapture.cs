...
public static class IconStudioCapture
{
    public static bool CaptureToPng(Camera camera, Transform stageRoot, SyntyEquipmentSlot slot, string fileName, out string savedPath)
    {
        savedPath = null;

#if !UNITY_EDITOR
        Debug.LogError("[IconStudio] PNG export is only supported in the Unity Editor.");
        return false;
#else
        if (!camera || !stageRoot)
            return false;

        var pixelSize = IconStudioSettings.GetOutputPixelSize(slot);
        var outputDirectory = IconStudioSettings.OutputRoot;
        EnsureDirectory(outputDirectory);

        savedPath = Path.Combine(outputDirectory, fileName).Replace('\\', '/');
        ...
        try
        {
            ...
            FitCamera(camera, stageRoot, slot, exportAspect, forExport: true);
            ...
            Debug.Log($"[IconStudio] Saved icon: {savedPath} ({pixelSize.x}x{pixelSize.y})");
            return true;
        }
        finally
        {
    ...
    public static void FitCameraForPreview(Camera camera, Transform stageRoot, SyntyEquipmentSlot slot)
    {
        if (!camera || !stageRoot)
            return;
        ...
        FitCamera(camera, stageRoot, slot, previewAspect, forExport: false);
    }

    static void FitCamera(Camera camera, Transform stageRoot, SyntyEquipmentSlot slot, float targetAspect, bool forExport)
    {
        ...
        var standOff = Mathf.Max(
            IconStudioSettings.MinCameraStandOff,
            worldBounds.extents.z + IconStudioSettings.NearPlaneMargin);
        ...
        var nearMargin = camera.nearClipPlane + IconStudioSettings.NearPlaneMargin;
        if (minZ < nearMargin)
        ...
        var padding = IconStudioSettings.GetFramePadding(slot);
        if (!forExport)
            padding += IconStudioSettings.PreviewExtraPadding;
    ...
    static List<Vector3> CollectRendererCorners(Transform stageRoot)
    {
        ...
        var hasActiveSkinned = false;

        foreach (var smr in stageRoot.GetComponentsInChildren<SkinnedMeshRenderer>(true))
        {
            if (!smr || !smr.enabled || !smr.gameObject.activeInHierarchy)
                continue;

            hasActiveSkinned = true;
            break;
        }

        if (hasActiveSkinned)
        {
            IconStudioSkinnedBounds.CollectWorldCorners(stageRoot, corners, onlyActiveRenderers: true);
            return corners;
        }
