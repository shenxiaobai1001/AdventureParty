...
namespace RPGCharacterAnims
{
    public class GUIControls : MonoBehaviour
    {
		...
		private void RollDodgeTurn()
		{
			...
			if (rpgCharacterController.HandlerExists(HandlerTypes.Dash)) {
				if (rpgCharacterController.CanStartAction(HandlerTypes.Dash)) {
					if (GUI.Button(new Rect(235, 115, 100, 30), "Dash Backward")) { rpgCharacterController.StartAction(HandlerTypes.Dash, DashType.Backward); }
					if (GUI.Button(new Rect(340, 115, 100, 30), "Dash Forward")) { rpgCharacterController.StartAction(HandlerTypes.Dash, DashType.Forward); }
					if (GUI.Button(new Rect(235, 145, 100, 30), "Dash Left")) { rpgCharacterController.StartAction(HandlerTypes.Dash, DashType.Left); }
					if (GUI.Button(new Rect(340, 145, 100, 30), "Dash Right")) { rpgCharacterController.StartAction(HandlerTypes.Dash, DashType.Right); }
				}
			}
