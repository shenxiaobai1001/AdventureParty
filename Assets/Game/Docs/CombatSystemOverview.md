...
**最后更新：** 2026-07-25  
**状态：** 武器=武艺数据层已定稿；Warrior/RPG Controller 可切换（Editor AssetDatabase）。**近战扫掠出伤 + 受击 + 战斗模拟（CombatSim/CombatBrain）+ XP 已接。** 移动 AI / 完整伪回合意图对抗 / 力量碾压尚未做。细节见 `DevHandoff.md`、`CombatSimAndBrainGaps.md`。
...
| ② 武器·武艺 | 剑/巨剑/锤…投掷（与武器类型同一套） | `WeaponProficiencyType` | 持该武器战斗涨对应熟练度；伤害/损耗 | “聪明地打” |
